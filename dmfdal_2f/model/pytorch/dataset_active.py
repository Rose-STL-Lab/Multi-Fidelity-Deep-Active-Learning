...
# Navier
import smoke_flow as smoke

class Navier: #to do list
    def __init__(self,):
        self.M = 2
        self.dim = 2
        
        self.fidelity_list = [32,64]
        self.single_query = smoke.single_query

        self.bounds = ((-0.05,0.05),(0.2,0.8))
    ...
    def query(self, X, m):
        ...
        N = X.shape[0]
        for n in range(N):
            # also the force field or only smoke resolution?
            y = self.single_query(X[n], self.fidelity_list[m])
            y_list.append(y)
            # print(n, "/", N)
        #
